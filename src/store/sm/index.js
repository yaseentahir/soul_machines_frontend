...
// const API_KEY = sessionStorage.getItem("my_api_key");
...
// if (AUTH_MODE === 0 && API_KEY === "")
//   startupErr = { msg: "REACT_APP_API_KEY not defined!" };
 ...
 * }
 */
export const animateCamera = createAsyncThunk(
  "sm/animateCamera",
  (/* { options, duration } */) => {
    if (!scene)
      return console.error("cannot animate camera, scene not initiated!");

    const serverControlledCameras = scene.hasServerControlledCameras();
    if (serverControlledCameras)
      return console.warn(
        "autonomous animation is active, manual camera animations are disabled!",
      );
...
export const createScene = createAsyncThunk(
  ...
  async (_, thunk) => {
    ...
    const { requestedMediaPerms, API_KEY } = thunk.getState().sm;
...
const smSlice = createSlice({
  ...
  reducers: {
    ...
    setHighlightMic: (state, { payload }) => ({
      ...state,
      highlightMic: payload.highlightMic,
    }),
    setHighlightMute: (state, { payload }) => ({
      ...state,
      highlightMute: payload.highlightMute,
    }),
    setHighlightChat: (state, { payload }) => ({
      ...state,
      highlightChat: payload.highlightChat,
    }),
    setHighlightMenu: (state, { payload }) => ({
      ...state,
      highlightMenu: payload.highlightMenu,
    }),
    setHighlightSkip: (state, { payload }) => ({
      ...state,
      highlightSkip: payload.highlightSkip,
    }),
    ...
    setRequestedMediaPerms: (state, { payload }) => {
      const requestedMediaPerms = {
        camera:
          "camera" in payload
            ? payload.camera
            : state.requestedMediaPerms.camera,
        mic: "mic" in payload ? payload.mic : state.requestedMediaPerms.mic,
        cameraDenied:
          "cameraDenied" in payload
            ? payload.cameraDenied
            : state.requestedMediaPerms.cameraDenied,
        micDenied:
          "micDenied" in payload
            ? payload.micDenied
            : state.requestedMediaPerms.micDenied,
      };
      sessionStorage.setItem(
        "requestedMediaPerms",
        JSON.stringify(requestedMediaPerms),
      );
      return {
        ...state,
        requestedMediaPerms,
    ...
    disconnect: (state) => {
      ...
      const lastUserMessage = transcript.filter(
      ...
      const presumeTimeout = timeDiff > 2400000;
      return {
    ...
    sendEvent: (state, { payload }) => {
      ...
      if (scene && persona) {
        persona.conversationSend(eventName, eventPayload || {}, {
          kind: kind || "event",
        });
      }
    },
