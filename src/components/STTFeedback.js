...
function STTFeedback({ className }) {
  ...
  const feedbackDisplay = (
    <span
      className={`badge bg-light input-display
        ...
        ${
          (transcriptOnlyText.length < 1 && intermediateUserUtterance === "") ||
          hideInputDisplay
            ? "hide-input"
            : "show-input"
        }
    ...
    >
      <div className="text-wrap text-start input-display">
        {userSpeaking ? `${characterName} heard: ` : "DP heard: "}
  ...
  return (
    <div className={className}>{hideInputDisplay ? null : feedbackDisplay}</div>
  );
}
