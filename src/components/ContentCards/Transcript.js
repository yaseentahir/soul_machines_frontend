    ...
    ({ source, text, card, timestamp }, index) => {
      ...
      return (
        <div key={timestamp}>
          ...
          >
            <div>
              <small>{source === "user" ? "You" : "Digital Person A"}</small>
            </div>
            <div className="transcript-entry-content">{text}</div>
          </div>
        </div>
      );
    },
    ...
    <div className={className}>
      <div className="transcript-list-group">
        {transcriptDisplay.length > 0 ? (
          ...
          <li className="list-group-item">No items to show, say something!</li>
        )}
